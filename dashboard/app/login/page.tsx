...
import { createClient as createSSRClient } from '@/lib/supabase';
import { Chrome, Mail, Loader2, Sparkles, CheckCircle2 } from 'lucide-react';
import { Suspense } from 'react';

// Vanilla Supabase client that uses localStorage for PKCE storage
// This is critical: @supabase/ssr's createBrowserClient uses cookies,
// and those cookies get wiped by the middleware before we can read them.
const getAuthClient = () => createSupabaseClient(
  ...
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
  {
    auth: {
      ...
      persistSession: true,
      // Uses localStorage by default — immune to cookie issues
    }
  }
);
...
function LoginContent() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      const ssrClient = createSSRClient();
      const { error } = await ssrClient.auth.signInWithOtp({
        email,
        options: {
